...
public class InvalidAreaRemove extends EntityComponent {
	public static final int ID = IDAssigner.getId();
	private double delay;
	private boolean enteredArea;
	private AABB area;

	public InvalidAreaRemove(Entity entity, AABB area) {
		super(entity, ID);
		this.delay = delay;
	...
	public void update(double delta) {
		if(getEntity().getAABB().intersects(area)) {
			if(!enteredArea) {
				enteredArea = true;
			}
			return;
